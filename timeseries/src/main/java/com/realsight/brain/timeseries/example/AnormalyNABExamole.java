...
public class AnormalyNABExamole {
	private int scope = 30;
	private double minValue;
	private double maxValue;
	private AnormalyNABExamole(String dir) {
	...
	public DoubleSeries detectorNABSeriesAnomaly(String dir) throws Exception{
		...
		TimeseriesData td = new TimeseriesData(dir);
		DoubleSeries nSeries = td.getPropertySeries("value");
		DoubleSeries anormalySeries = td.getPropertySeries("label");
		int n = nSeries.size();
...
//		System.out.println(this.scope);
		...
		for(File file : root.listFiles()){
			if(file.isDirectory()){
				for(File dir : file.listFiles()){
					...
					sin.close();
					AnormalyNABExamole htm = new AnormalyNABExamole(dir.getPath());
	...
	public static void main(String[] args) throws Exception {
		...
		AnormalyNABExamole.run(nabPath);
	}
}
