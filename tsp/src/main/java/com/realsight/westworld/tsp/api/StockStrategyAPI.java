	...
	public void run(MultipleDoubleSeries nSeries) {
		...
		for ( int i = 0; i < nSeries.size(); i++ ) {
			...
			if ( i < nSeries.size()*19/20 ) continue;
				...
				if (score > 0.75) {
					scores.add(new TimeSeries.Entry<Double>(1.0, timestamp));
...
//			System.out.println("acc - > " + (1.0*acc/sum) + ", up -> " + (up/(up+down)));
			S.add(new TimeSeries.Entry<Double>(today_price.get(0, 0), timestamp));
			Date date = new Date(timestamp);
			System.out.println(date.toString() + "\t" + stock.totalAssets(today_price.get(0, 0)) + "\t" + stock + "\t" + today_price.get(0, 0) + "\t" + tmp_p);
			AssetsSeries.add(new TimeSeries.Entry<Double>(stock.totalAssets(today_price.get(0, 0)), timestamp));
			nums.add(new TimeSeries.Entry<Double>(stock.getNumStock()+0.0, timestamp));
		}
		S.normly2();
