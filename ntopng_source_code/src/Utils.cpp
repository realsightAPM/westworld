...
bool Utils::postHTTPJsonData(char *username, char *password, char *url, char *json) {
  ...
  string json_data = "{" + key + ":" + value+"}";
  char *json_data_p = json_data.c_str();
  /*
  ...
  // create  url for post
  string post_url = new string(url);
  post_url=post_url + "update?" + "_=" + t + "&boost=1.0&commitWithin=1000&overwrite=true&wt=json";
  char *post_url_p = post_url.c_tr();
  ...
  if(curl) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, post_url_p);
    ...
    curl_easy_setopt(curl, CURLOPT_POSTFIELDS, json_data_p);
    curl_easy_setopt(curl, CURLOPT_POSTFIELDSIZE, (long)strlen(json_data_p));
    ...
    if(res != CURLE_OK) {
      ...
      ntop->getTrace()->traceEvent(TRACE_INFO, "Posted JSON to %s", post_url_p);
